...
rule stranded_fiveprime_bed:
    ...
    container:
        "docker://quay.io/biocontainers/datamash:1.9"
    shell:
        """
        ...
        if [[ "{wildcards.direction}" == "plus" ]]; then
            awk 'BEGIN{{OFS="\\t"}} $6 == "+" {{print $1, $2, $2+1, $4, $5, $6}}' {input} \
                | datamash -g 1,2,3 count 4 sum 5 unique 6 > results/fiveprime/{wildcards.sample}_plus.fiveprime.bed
        else
            awk 'BEGIN{{OFS="\\t"}} $6 == "-" {{print $1, $3-1, $3, $4, $5, $6}}' {input} \
                | datamash -g 1,2,3 count 4 sum 5 unique 6 > results/fiveprime/{wildcards.sample}_minus.fiveprime.bed
        fi
        """
...
rule filtered_fiveprime_background:
    ...
    params:
        kernal_size=config["call_tss"]["kernal_size"],
    message:
